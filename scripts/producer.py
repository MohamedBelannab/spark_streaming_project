...
def main():
    ...
    try:
        ...
        # En mode non-interactif (nohup ou --no-interactive), utiliser la génération aléatoire
        import sys
        if '--no-interactive' in sys.argv or not sys.stdin.isatty():
            print("🔄 Mode non-interactif, génération aléatoire activée")
            use_csv = False
        else:
